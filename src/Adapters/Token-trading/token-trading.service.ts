  ...
  constructor() {
    ...
    addWallets(walletNumbers: number[]) {
        for (const walletNumber of walletNumbers) {
          ...
          const existingWallet = this.wallets.find((wallet) => wallet.address === NewWallet.address);
...
async approveToken(tokenAddress: string, walletNumbers: number[], gwei: number = 3): Promise<string[]> {
      ...
      promises.push(
        new Promise(async (resolve, reject) => {
          try {
            ...
            console.log(`Balance in Wallet ${walletNumber}:`, balance.toString());
            ...
            const tx = await router2.swapExactTokensForETHSupportingFeeOnTransferTokens(
              balance,
              amountOutMin.toString(),
              path,
              wallet.address,
